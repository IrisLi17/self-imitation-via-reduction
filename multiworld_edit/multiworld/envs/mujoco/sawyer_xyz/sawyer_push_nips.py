...
class SawyerPushAndReachXYEnv(MujocoEnv, Serializable, MultitaskEnv):
    ...
    def _get_info(self):
        ...
        return dict(
           ...
           # ),
            ...
            is_success = float( (puck_distance_l2  < 0.06) and (hand_distance_l2 < 0.03)),
    ...
    def compute_rewards(self, actions, obs, prev_obs=None, reward_type=None):
        ...
        hand_dist_2 = np.linalg.norm(hand_goals - hand_pos, ord=2, axis=1)
        puck_dist_2 = np.linalg.norm(puck_goals - puck_pos, ord=2, axis=1)
        if reward_type == 'hand_distance':
            r = -hand_distances
        elif reward_type == 'hand_puck_success':
            #print('hand_goals',hand_goals)
            #print('hand_dist_2',hand_dist_2)
            #print('hand_success',hand_dist_2<0.03)
            #print('hand_puck_success',(puck_dist_2<0.06) and (hand_dist_2<0.03))
            #print('type of hand_goal_2',type(hand_dist_2))
            r = -np.logical_not( (puck_dist_2  < 0.06) and (hand_dist_2 < 0.03) ).astype(float) 
    ...
    def _reset_hand(self):
        ...
        #print('reset_target_hand_pos',new_mocap_pos)
        for _ in range(self.num_mocap_calls_for_reset): #10
            self.data.set_mocap_pos('mocap', new_mocap_pos)
            self.data.set_mocap_quat('mocap', np.array([1, 0, 1, 0]))
            self.do_simulation(None, self.frame_skip)
       # print('reset_after_sim_hand_pos',self.get_endeff_pos())
       # print('l2_diff',np.linalg.norm(new_mocap_pos-self.get_endeff_pos()))
