...
class SAC_sanity(OffPolicyRLModel):
    """
    ...
    def __init__(self, policy, env, trained_sac_model=None, gamma=0.99, learning_rate=3e-4, buffer_size=50000,
                 ...
                 hack_augment_policy=False, double_buffer=True,
                 verbose=0, tensorboard_log=None,
                 _init_setup_model=True, policy_kwargs=None, full_tensorboard_log=False):

        super(SAC_sanity, self).__init__(policy=policy, env=env, replay_buffer=None, verbose=verbose,
    ...
    def setup_model(self):
        with SetVerbosity(self.verbose):
            ...
            with self.graph.as_default():
                ...
                # Actually we don't need an augment replay buffer that large.
    ...
    def log_traj(self, augment_episode_buffer):
    ...
    def fill_augment_buffer(self):
        ...
        while augment_episode < 5:
            ...
            step_so_far = 0
            augment_buf = []
            start_state = self.env.env.sim.get_state()
            for _ in range(30):
                action = self.trained_sac_model.policy_tf.step(obs[None],deterministic=False).flatten()
                rescaled_action = action * np.abs(self.action_space.low)
                next_obs, reward, done, info = self.env.step(rescaled_action)
                if done:
                    break
                ...
                obs = next_obs
            if not done:
                ultimate_goal = obs[45:50].copy()
                noise = np.random.uniform(low=-0.10, high=0.10, size=(100, 2))
                noise = np.sign(noise) * 0.05 + noise
                obstacle_pos = np.expand_dims(obs[6:8], axis=0) + noise
                assert obstacle_pos.shape == (100, 2)
                perturb_obs = np.tile(obs[:], (obstacle_pos.shape[0], 1))
                perturb_obs[:, 6:8] = obstacle_pos
                perturb_obs[:, 12:14] = perturb_obs[:, 6:8] - perturb_obs[:, 0:2]
                feed_dict = {self.trained_sac_model.observations_ph: perturb_obs}
                perturb_values = np.squeeze(self.trained_sac_model.sess.run(self.trained_sac_model.step_ops[6], feed_dict), axis=-1)
                subgoal_obs = np.tile(obs[:], (obstacle_pos.shape[0], 1))
                ...
                best_idx = np.argmax(
                    ((perturb_values - np.min(perturb_values)) / (np.max(perturb_values - np.min(perturb_values))))
                    * (subgoal_values - np.min(subgoal_values)) / (np.max(subgoal_values) - np.min(subgoal_values)))
                best_subgoal = obstacle_pos[best_idx]

                self.env.env.goal[3:] = np.array([0., 1.])
                self.env.env.goal[0:3] = obs[6:9]
                self.env.env.goal[0:2] = best_subgoal
                ...
                while len(augment_buf) < self.env.env.spec.max_episode_steps:
                    action = self.trained_sac_model.policy_tf.step(obs[None],deterministic=False).flatten()
                    rescaled_action = action * np.abs(self.action_space.low)
                    next_obs, reward, _, info = self.env.step(rescaled_action)
                ...
                if len(augment_buf) < self.env.env.spec.max_episode_steps:
                    ...
                    if info['is_success']:
                        ...
                        assert abs(np.sum([item[-1] for item in augment_buf]) - 1) < 1e-4
                        assert abs(augment_buf[-1][-1] - 1) < 1e-4
                        assert abs(np.sum([item[2] for item in augment_buf]) - 2) < 1e-4
                        for item in augment_buf:
                            self.augment_replay_buffer.add(*item)
                        print('augment episode', augment_episode)
                        augment_episode += 1
    ...
    def learn(self, total_timesteps, callback=None, seed=None,
        ...
        with SetVerbosity(self.verbose), TensorboardWriter(self.graph, self.tensorboard_log, tb_log_name, new_tb_log) \
            ...
            # Fill augment_replay_buffer with some specific hard case.
            self.fill_augment_buffer()
            ...
            for step in range(total_timesteps):
                ...
                state_buf.append(self.env.env.sim.get_state())
                obs_buf.append(obs)
                new_obs, reward, done, info = self.env.step(rescaled_action)
                transition_buf.append((obs, action, reward, new_obs, float(done)))
                ...
                #     subgoal_obs[:, 40:43] = subgoal_obs[:, 6:9]
                #     subgoal_obs[:, 43:45] = np.array([[0., 1.]])
                #     subgoal_obs[:, 45:47] = perturbed_obstacle_pos
                #     subgoal_obs[:, 47:48] = subgoal_obs[:, 42: 43]
                #     subgoal_obs[:, 48:50] = np.array([[0., 1.]])
                #
                #     # Evaluate perturb_obs -> ultimate goal
                #     feed_dict = {self.trained_sac_model.observations_ph: perturb_obs}
                #     perturb_values = np.squeeze(self.trained_sac_model.sess.run(self.trained_sac_model.step_ops[6], feed_dict), axis=-1)
                #     # Evaluate current obs -> perturbation
                #     feed_dict = {self.trained_sac_model.observations_ph: subgoal_obs}
                #     subgoal_values = np.squeeze(self.trained_sac_model.sess.run(self.trained_sac_model.step_ops[6], feed_dict), axis=-1)
                #
                ...
                #     selected_restart_obs = restart_obs[selected_idx]
                #     selected_obstacle_pos = perturbed_obstacle_pos[selected_idx]
                #     # print('selected obstacle pos', selected_obstacle_pos)
                ...
                #         # Set the goal to perturbed obstacle pos
                #         self.env.env.goal[3:] = np.array([0., 1.])
                #         self.env.env.goal[2] = self.env.env.sim.data.get_site_xpos('object1')[2]
                #         self.env.env.goal[0:2] = selected_obstacle_pos[augment_idx]
                ...
                #             # not needed in general)
                #             if self.action_noise is not None:
                #                 augment_action = np.clip(augment_action + self.action_noise(), -1, 1)
                #             # Rescale from [-1, 1] to the correct bounds
                ...
                #         if len(augment_episode_buffer) < self.env.env.spec.max_episode_steps:
                #             assert augment_info['is_success'], str(len(augment_episode_buffer))
                #             self.env.needs_reset = False
                #             self.env.env.goal[:] = selected_restart_obs[augment_idx][-5:]
