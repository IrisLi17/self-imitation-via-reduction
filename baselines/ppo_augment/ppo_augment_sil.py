...
from utils.replay_buffer import SumRPrioritizedReplayBuffer


class PPO2_augment_sil(ActorCriticRLModel):
    """
    ...
    def __init__(self, policy, env, eval_env=None, gamma=0.99, n_steps=128, ent_coef=0.01, learning_rate=2.5e-4,
                 vf_coef=0.5, max_grad_norm=0.5, lam=0.95, nminibatches=4, noptepochs=4, cliprange=0.2,
                 cliprange_vf=None, parallel=False, aug_adv_weight=1.0, curriculum=False,
                 verbose=0, tensorboard_log=None, _init_setup_model=True,
                 policy_kwargs=None, full_tensorboard_log=False):

        super(PPO2_augment_sil, self).__init__(policy=policy, env=env, verbose=verbose, requires_vec_env=True,
        ...
        self.aug_replay = None
        self.importance_weight_ph = None
        self._sil_train = None
    ...
    def setup_model(self):
        with SetVerbosity(self.verbose):
            ...
            self.aug_replay = SumRPrioritizedReplayBuffer(int(1e5), alpha=0.6)
            ...
            with self.graph.as_default():
                ...
                with tf.variable_scope("loss", reuse=False):
                    ...
                    self.aug_entropy = tf.reduce_sum(self.importance_weight_ph * mask * tf.reduce_mean(
                        train_model.proba_distribution.entropy(), axis=-1)) / num_valid_samples
                    ...
                    self.aug_vf_loss = .5 * tf.reduce_sum(self.importance_weight_ph * mask * vf_losses1) / num_valid_samples
                    ...
                    sil_grads = tf.gradients(sil_loss, self.params)
                    if self.max_grad_norm is not None:
                        sil_grads, _grad_norm = tf.clip_by_global_norm(sil_grads, self.max_grad_norm)
                    sil_grads = list(zip(sil_grads, self.params))
                trainer = tf.train.AdamOptimizer(learning_rate=self.learning_rate_ph, epsilon=1e-5)
                self._train = trainer.apply_gradients(grads)
                self._sil_train = trainer.apply_gradients(sil_grads)
                ...
                self.aug_neglogpac_op = neglogpac
    ...
    def learn(self, total_timesteps, callback=None, seed=None, log_interval=1, tb_log_name="PPO2",
        ...
        with SetVerbosity(self.verbose), TensorboardWriter(self.graph, self.tensorboard_log, tb_log_name, new_tb_log) \
            ...
            from baselines.ppo_augment.sil_priority_runner import SILPriorityRunner
            runner = SILPriorityRunner(env=self.env, model=self, n_steps=self.n_steps,
            ...
            for update in range(1, n_updates + 1):
                ...
                #         'MasspointPush' in self.env.get_attr('spec')[0].id and update > 50)):
