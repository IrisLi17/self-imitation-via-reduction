...
class SPDDPG(OffPolicyRLModel):
    """
    Deep Deterministic Policy Gradient (DDPG) model + Shortest path
    ...
    def __init__(self, policy, env, gamma=0.99, memory_policy=None, eval_env=None, nb_train_steps=50,
        ...
        self.gamma = gamma
        self.tau = tau
        self.bias = np.sum(np.power(gamma, np.arange(0, self.env.env.spec.max_episode_steps)))
        ...
        self.nb_subgoal_candidates = nb_subgoal_candidates
